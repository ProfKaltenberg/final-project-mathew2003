...
FinalData <- read.csv("FinalProjectdataset.csv")
...
This is my baseline OLS regression using all four variables on the full county-level dataset. I regress 2-bedroom Fair Market Rent on vacancy rate, total vacant units, and median income.
...
Here I filter the dataset to only include counties where median household income is above $100,000. I want to see whether the relationship between vacancy rates and rent looks different in wealthier counties compared to the full sample.
...
My preferred regression uses the full dataset but log-transforms median income. Income tends to have a non-linear relationship with rent — an extra dollar of income matters more at lower income levels than at higher ones. Log-transforming income captures this and typically improves model fit.

```{r}
# Create a log income variable
FinalData <- mutate(FinalData, log_income = log(median_income))

# Preferred regression with log-transformed income
reg_preferred <- lm(fmr_2bed ~ vacancy_rate + total_housing_units + log_income + households_with_children, data = FinalData)
...
stargazer(reg_full, reg_filtered, reg_preferred,
          ...
          covariate.labels = c("Vacancy Rate (%)",
                               "Total Vacant Units",
                               "Median Income ($)",
                               "Log Median Income"),
          column.labels    = c("Full Sample", "High Income Counties", "Preferred (Log Income)"),
          omit.stat        = c("f", "ser"),
          digits           = 3,
